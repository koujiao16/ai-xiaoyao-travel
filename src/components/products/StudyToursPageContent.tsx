...
function VideoStoryCard({ story }: { story: StudyToursVideoStory }) {
  ...
  // e.g. /images/videos/video-xian-study-tour.jpg or /images/videos/video-harbin-study-tour.jpg
  return (
...
function ResourceCard({
  ...
  resource: StudyToursResource;
  available: boolean;
}) {
  // Future PDF download: resource.pdfPath
  // e.g. /resources/xian-study-tour-4-day-program.pdf
  return (
...
export function StudyToursPageContent({ locale }: { locale: SiteLocale }) {
  const content = getStudyToursContent(locale);
