...
import {
  getInboundGroupsContent,
  type InboundGroupCasePlaceholder,
} from "@/lib/inbound-groups-content";

function CasePlaceholderCard({ item }: { item: InboundGroupCasePlaceholder }) {
  // Future image: item.imagePath
  // Future video: item.videoUrl (WeChat Channels or external link)
  return (
...
export function InboundGroupsPageContent({ locale }: { locale: SiteLocale }) {
  const content = getInboundGroupsContent(locale);
  ...
  return (
    <main>
      ...
      <Section tone="ivory">
        <Container>
          <Title title={content.marketsWeSupport.title} />
          <div className="mt-8 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {content.marketsWeSupport.items.map((item) => (
      ...
      <Section tone="navy">
        <Container>
          <Title title={content.casePlaceholders.title} invert />
          <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {content.casePlaceholders.items.map((item) => (
              <CasePlaceholderCard key={item.title} item={item} />
            ))}
          </div>
        </Container>
      </Section>
