...
import {
  getGroundOperationsContent,
  type GroundOperationsMediaPlaceholder,
} from "@/lib/ground-operations-content";

function MediaPlaceholderCard({ item }: { item: GroundOperationsMediaPlaceholder }) {
  // Future image: item.imagePath
  // e.g. /images/operations/fleet.jpg, guides.jpg, group-reception.jpg, hotel-dining.jpg
  return (
    <div className="overflow-hidden rounded-xl2 border border-white/10 bg-white/[0.04] shadow-card">
      <div className="relative aspect-[4/3] overflow-hidden bg-gradient-to-br from-navy-900/80 via-navy-950 to-navy-900">
        ...
        <div className="absolute inset-0 flex items-center justify-center px-4">
          <span className="text-center text-[10px] tracking-[0.14em] uppercase text-ivory-100/40">
            {item.label}
          </span>
        </div>
      </div>
...
export function GroundOperationsPageContent({ locale }: { locale: SiteLocale }) {
  const content = getGroundOperationsContent(locale);
  ...
  return (
    <main>
      ...
      <Section tone="ivory">
        <Container>
          <Title title={content.operationCapabilities.title} />
          <div className="mt-8 grid gap-4 lg:grid-cols-2">
            {content.operationCapabilities.items.map((item) => (
              <FeatureCard
                key={item.title}
                tone="ivory"
                title={item.title}
