...
import {
  getSpecialInterestTravelContent,
  type SpecialInterestMediaPlaceholder,
} from "@/lib/special-interest-travel-content";

function MediaPlaceholderCard({ item }: { item: SpecialInterestMediaPlaceholder }) {
  // Future image: item.imagePath
  // e.g. /images/special-interest/qinling-exploration.jpg, qinling-hiking.jpg, etc.
  return (
...
export function SpecialInterestTravelPageContent({ locale }: { locale: SiteLocale }) {
  const content = getSpecialInterestTravelContent(locale);
  ...
  return (
    <main>
      ...
      <Section tone="ivory">
        <Container>
          <Title title={content.mediaPlaceholders.title} />
          <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
